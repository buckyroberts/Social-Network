...
$tempFileList = scandir(DIR_FS_TRADE_IMG_TMP);
...
if(count($tempFileList) > 0){
    foreach($tempFileList as $fileName){
        ...
        if(file_exists(DIR_FS_TRADE_IMG_TMP . $fileName)){
            if(time() - filemtime(DIR_FS_TRADE_IMG_TMP . $fileName) > 1 * 24 * 3600){
                @unlink(DIR_FS_TRADE_IMG_TMP . $fileName);
            }
        }
