...
(function ($, window, document){
    ...
    var _templates = {
        ...
        image: '<div><label for="link">{url}</label> <input type="text" id="image" value="http://" /></div>' + '<div><input type="button" class="button" value="{insert}" /></div>' + '<hr>' + '<div class="sceditor-image-uploader-cont"><label for="width">{uploadimage}</label> <span class="inputholder"> <input id="sceditor_image_upload" name="sceditor_image_upload" type="file" /> </span></div>' + '<hr>' + '<div><label for="width">{width}</label> <input type="text" id="width" size="2" /></div>' + '<div><label for="height">{height}</label> <input type="text" id="height" size="2" /></div>',
    ...
    $.sceditor = function (el, options){
        ...
        initEvents = function (){
            ...
            $(original.form).bind('reset', handleFormReset).submit(base.updateOriginal);

            $(window).bind('resize orientationChanged', handleWindowResize);

            $wysiwygBody.keypress(handleKeyPress).keydown(handleKeyDown).keydown(handleBackSpace).keyup(appendNewLine).bind('blur', valueChangedBlur).keyup(valueChangedKeyUp).bind('paste', handlePasteEvt).bind(IE_VER ? 'selectionchange' : 'keyup focus blur contextmenu mouseup touchend click', checkSelectionChanged).bind('keydown keyup keypress focus blur contextmenu', handleEvent);

            if(options.emoticonsCompat && window.getSelection)
                $wysiwygBody.keyup(emoticonsCheckWhitespace);

            $sourceEditor.bind('blur', valueChangedBlur).keyup(valueChangedKeyUp).bind('keydown keyup keypress focus blur contextmenu', handleEvent).keydown(handleKeyDown);

            $wysiwygDoc.mousedown(handleMouseDown).bind('blur', valueChangedBlur).bind(IE_VER ? 'selectionchange' : 'focus blur contextmenu mouseup click', checkSelectionChanged).bind('beforedeactivate keyup', saveRange).keyup(appendNewLine).focus(function (){
                lastRange = null;
            });

            $editorContainer.bind('selectionchanged', checkNodeChanged).bind('selectionchanged', updateActiveButtons).bind('selectionchanged valuechanged nodechanged', handleEvent);
        };
        ...
        initToolBar = function (){
            var $group, $button, shortcutName, commands = base.commands, exclude = (options.toolbarExclude || '').split(','), groups = options.toolbar.split('|');
            ...
            $.each(groups, function (idx, group){
                ...
                $.each(group.split(','), function (idx, button){
                    ...
                    $button = _tmpl('toolbarButton', {
                        name: button, dispName: base._(commands[button].tooltip || button) + shortcutName
                    }, true);
         ...
         */
        initResize = function (){
            var minHeight, maxHeight, minWidth, maxWidth, mouseMoveFunc, mouseUpFunc, $grip = $('<div class="sceditor-grip" />'), // cover is used to cover the editor iframe so document still gets mouse move events
                $cover = $('<div class="sceditor-resize-cover" />'), startX = 0, startY = 0, startWidth = 0, startHeight = 0, origWidth = $editorContainer.width(), origHeight = $editorContainer.height(), dragging = false, rtl = base.rtl();
            ...
            mouseMoveFunc = function (e){
                ...
                var newHeight = startHeight + (e.pageY - startY), newWidth = rtl ? startWidth - (e.pageX - startX) : startWidth + (e.pageX - startX);
        ...
        initEmoticons = function (){
            ...
            $.each(emoticons, function (idx, val){
                $.each(val, function (key, url){
                    ...
                    if(root){
                        url = {
                            url: root + (url.url || url), tooltip: url.tooltip || key
                        };
         ...
         */
        autofocus = function (){
            var rng, elm, txtPos, doc = $wysiwygDoc[0], body = $wysiwygBody[0], focusEnd = !!options.autofocusEnd;
            ...
            if(base.sourceMode()){
                ...
                if(sourceEditor.setSelectionRange)
                    sourceEditor.setSelectionRange(txtPos, txtPos);else
                    if(sourceEditor.createTextRange){
        ...
        updateToolBar = function (disable){
            ...
            $toolbar.find('.sceditor-button').removeClass('disabled').each(function (){
                ...
                if(disable === true || (inSourceMode && !button.data('sceditor-txtmode')))
                    ...
                    if(!inSourceMode && !button.data('sceditor-wysiwygmode'))
                        button.addClass('disabled');
            });
        };
        ...
        base.expandToContent = function (ignoreMaxHeight){
            var currentHeight = $editorContainer.height(), height = $wysiwygBody[0].scrollHeight || $wysiwygDoc[0].documentElement.scrollHeight, padding = (currentHeight - $wysiwygEditor.height()), maxHeight = options.resizeMaxHeight || ((options.height || $original.height()) * 2);
        ...
        base.destroy = function (){
            ...
            $(original.form).unbind('reset', handleFormReset).unbind('submit', base.updateOriginal);
        ...
        base.createDropDown = function (menuItem, dropDownName, content, ieUnselectable){
            ...
            var dropDownCss, onlyclose = $dropdown && $dropdown.is('.sceditor-' + dropDownName);
            ...
            dropDownCss = {
                top: menuItem.offset().top + 6, left: menuItem.offset().left, marginTop: menuItem.outerHeight()
            };
            $.extend(dropDownCss, options.dropDownCss);

            $dropdown = $('<div class="sceditor-dropdown sceditor-' + dropDownName + '" />').css(dropDownCss).append(content).appendTo($('body')).on('click focusin', function (e){
        ...
        handlePasteEvt = function (e){
            var html, handlePaste, scrollTop, elm = $wysiwygBody[0], doc = $wysiwygDoc[0], checkCount = 0, pastearea = document.createElement('div'), prePasteContent = doc.createDocumentFragment();
            ...
            handlePaste = function (elm, pastearea){
                if(elm.childNodes.length > 0 || checkCount > 25){
                    ...
                    if(pastearea.childNodes.length > 0)
                        handlePasteData(elm, pastearea);else
        ...
        base.sourceEditorInsertText = function (text, endText){
            ...
            // IE < 9
                ...
                if(typeof document.selection.createRange !== 'undefined'){
        ...
        base.val = function (val, filter){
            ...
            return base.inSourceMode() ? base.getSourceEditorValue(false) : base.getWysiwygEditorValue();
        };
        ...
        base.insert = function (start, end, filter, convertEmoticons, allowMixed){
            if(!base.inSourceMode()){
                ...
                if(end){
                    var html = base.getRangeHelper().selectedHtml(), frag = $('<div>').appendTo($('body')).hide().html(html);
        ...
        base.getWysiwygEditorValue = function (filter){
            var html, ieBookmark, hasSelection = rangeHelper.hasSelection();
            ...
            // and restore it after
                ...
                if(lastRange && lastRange.getBookmark)
        ...
        replaceEmoticons = function (node){
            ...
            var doc = node.ownerDocument, emoticonCodes = [], emoticonRegex = [], emoticons = $.extend({}, options.emoticons.more, options.emoticons.dropdown, options.emoticons.hidden);
            ...
            (function convertEmoticons(node){
                ...
                while(node != null){
                    var parts, key, emoticon, parsedHtml, emoticonIdx, nextSibling, startIdx, nodeParent = node.parentNode, nodeValue = node.nodeValue;
                            ...
                            while(emoticonIdx--){
                                ...
                                if(startIdx > -1){
                                    ...
                                    parsedHtml = $.sceditor.dom.parseHTML(_tmpl('emoticon', {
                                        key: key, url: emoticon.url || emoticon, tooltip: emoticon.tooltip || key
                                    }), doc);
        ...
        sourceEditorSelectedText = function (){
            ...
            if(sourceEditor.selectionStart != null)
                return sourceEditor.value.substring(sourceEditor.selectionStart, sourceEditor.selectionEnd);else
        ...
        base.execCommand = function (command, param){
            var executed = false, $parentNode = $(rangeHelper.parentNode());
        ...
        updateActiveButtons = function (e){
            var state, stateHandler, firstBlock, $button, parent, doc = $wysiwygDoc[0], i = btnStateHandlers.length, inSourceMode = base.sourceMode();
        ...
        appendNewLine = function (){
            ...
            $.sceditor.dom.rTraverse($wysiwygBody[0], function (node){
                ...
                if((node.nodeType === 3 && !/^\s*$/.test(node.nodeValue)) || name === 'br' || (IE_BR_FIX && !node.firstChild && !$.sceditor.dom.isInline(node, false))){
        ...
        handleWindowResize = function (){
            var height = options.height, width = options.width;
        ...
        handleEvent = function (e){
            var customEvent, clone = $.extend({}, e);
        ...
        base.blur = function (handler, excludeWysiwyg, excludeSource){
            if($.isFunction(handler))
                base.bind('blur', handler, excludeWysiwyg, excludeSource);else
                if(!base.sourceMode()){
        ...
        emoticonsKeyPress = function (e){
            var pos = 0, curChar = String.fromCharCode(e.which);
            ...
            if(!base.emoticonsCache){
                ...
                $.each($.extend({}, options.emoticons.more, options.emoticons.dropdown, options.emoticons.hidden), function (key, url){
                    base.emoticonsCache[pos++] = [key, _tmpl('emoticon', {
                        key: key, url: url.url || url, tooltip: url.tooltip || key
                    })];
                });
        ...
        emoticonsCheckWhitespace = function (){
            ...
            currentEmoticons = $.map(currentEmoticons, function (emoticon){
                ...
                if(rangeStartContainer === next)
                    rangeStart = previousText.length + range.startOffset;else
                    if(rangeStartContainer === currentBlock && currentBlock.childNodes[range.startOffset] === next)
                        rangeStart = previousText.length;else
        ...
        handleKeyDown = function (e){
            ...
            if(e.shiftKey){
                ...
                if(numpad_shift_keys[which])
                    character = numpad_shift_keys[which];else
    ...
    $.sceditor.isWysiwygSupported = (function (){
        var match, isUnsupported, contentEditable = $('<div contenteditable="true">')[0].contentEditable, contentEditableSupported = typeof contentEditable !== 'undefined' && contentEditable !== 'inherit', userAgent = navigator.userAgent;
    ...
    $.sceditor.escapeEntities = function (str, quote){
        ...
        str = str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/ {2}/g, ' &nbsp;').replace(/\r\n|\r/g, '\n').replace(/\n/g, '<br />');
    ...
    $.sceditor.escapeUriScheme = function (url){
        var path, validSchemes = /^(?:https?|s?ftp|mailto|spotify|skype|ssh|teamspeak|tel):|(?:\/\/)/i, // If there is a : before a / then it has a scheme
            hasScheme = /^[^\/]*:/i, location = window.location;
    ...
    $.sceditor.commands = {
        // START_COMMAND: Bold
        bold: {
            exec: 'bold', tooltip: 'Bold', shortcut: 'ctrl+b'
        }, // END_COMMAND
        // START_COMMAND: Italic
        italic: {
            exec: 'italic', tooltip: 'Italic', shortcut: 'ctrl+i'
        }, // END_COMMAND
        // START_COMMAND: Underline
        underline: {
            exec: 'underline', tooltip: 'Underline', shortcut: 'ctrl+u'
        }, // END_COMMAND
        // START_COMMAND: Strikethrough
        strike: {
            exec: 'strikethrough', tooltip: 'Strikethrough'
        }, // END_COMMAND
        // START_COMMAND: Subscript
        subscript: {
            exec: 'subscript', tooltip: 'Subscript'
        }, // END_COMMAND
        // START_COMMAND: Superscript
        superscript: {
            exec: 'superscript', tooltip: 'Superscript'
        }, // END_COMMAND
        ...
        center: {
            exec: 'justifycenter', tooltip: 'Center'
        }, // END_COMMAND
        // START_COMMAND: Right
        right: {
        ...
        justify: {
            exec: 'justifyfull', tooltip: 'Justify'
        }, // END_COMMAND

        // START_COMMAND: Font
        font: {
            _dropDown: function (editor, caller, callback){
                var fonts = editor.opts.fonts.split(','), content = $('<div />'), /** @private */
                clickFunc = function (){
                ...
                $.sceditor.command.get('font')._dropDown(editor, caller, function (fontName){
        ...
        color: {
            _dropDown: function (editor, caller, callback){
                var i, x, color, colors, genColor = {
                        r: 255, g: 255, b: 255
                    }, content = $('<div />'), colorColumns = editor.opts.colors ? editor.opts.colors.split('|') : new Array(21), // IE is slow at string concation so use an array
                    html = [], cmd = $.sceditor.command.get('color');
        ...
        removeformat: {
            exec: 'removeformat', tooltip: 'Remove Formatting'
        }, // END_COMMAND
        ...
        pastetext: {
            exec: function (caller){
                var val, editor = this, content = _tmpl('pastetext', {
                    label: editor._('Paste your text inside the following box:'), insert: editor._('Insert')
                }, true);
        ...
        bulletlist: {
            exec: 'insertunorderedlist', tooltip: 'Bullet list'
        }, // END_COMMAND
        // START_COMMAND: Ordered List
        orderedlist: {
            exec: 'insertorderedlist', tooltip: 'Numbered list'
        }, // END_COMMAND
        ...
        indent: {
            state: function (parents, firstBlock){
                ...
                var range, startContainerParent, endContainerParent, $firstBlock = $(firstBlock), parentLists = $firstBlock.parents('ul,ol,menu'), parentList = parentLists.first();
                ...
                if($firstBlock.is('ul,ol,menu')){
                    ...
                    if(window.Range && range instanceof Range){
                        ...
                        if(startContainerParent !== startContainerParent.parentNode.firstElementChild || // work around a bug in FF
                ...
                var editor = this, $elm = $(editor.getRangeHelper().getFirstBlockParent());
        ...
        table: {
            exec: function (caller){
                var editor = this, content = _tmpl('table', {
                    rows: editor._('Rows:'), cols: editor._('Cols:'), insert: editor._('Insert')
                }, true);

                content.find('.button').click(function (e){
                    var IE_VER = $.sceditor.ie;
                    var rows = content.find('#rows').val() - 0, cols = content.find('#cols').val() - 0, html = '<table>';
        ...
        horizontalrule: {
            exec: 'inserthorizontalrule', tooltip: 'Insert a horizontal rule'
        }, // END_COMMAND
        ...
        image: {
            exec: function (caller){
                ...
                var editor = this, content = _tmpl('image', {
                    url: editor._('URL:'),
                ...
                content.find('.button').click(function (e){
                    var val = content.find('#image').val(), width = content.find('#width').val(), height = content.find('#height').val(), attrs = '';
        ...
        email: {
            exec: function (caller){
                var editor = this, content = _tmpl('email', {
                    label: editor._('E-mail:'), insert: editor._('Insert')
                }, true);
        ...
        link: {
            exec: function (caller){
                var editor = this, content = _tmpl('link', {
                    url: editor._('URL:'), desc: editor._('Description (optional):'), ins: editor._('Insert')
                }, true);

                content.find('.button').click(function (e){
                    var val = content.find('#link').val(), description = content.find('#des').val();
        ...
        unlink: {
            state: function (){
                ...
                var $current = $(this.currentNode()), $anchor = $current.is('a') ? $current : $current.parents('a').first();
        ...
        // START_COMMAND: Quote
        quote: {
            forceNewLineAfter: ['blockquote'], exec: function (caller, html, author){
                var IE_VER = $.sceditor.ie;
        ...
        emoticon: {
            exec: function (caller){
                ...
                var createContent = function (includeMore){
                    var emoticonsCompat = editor.opts.emoticonsCompat, rangeHelper = editor.getRangeHelper(), startSpace = emoticonsCompat && rangeHelper.getOuterText(true, 1) !== ' ' ? ' ' : '', endSpace = emoticonsCompat && rangeHelper.getOuterText(false, 1) !== ' ' ? ' ' : '', $content = $('<div />'), $line = $('<div />').appendTo($content), emoticons = $.extend({}, editor.opts.emoticons.dropdown, includeMore ? editor.opts.emoticons.more : {}), perLine = 0;
                    ...
                    $.each(emoticons, function (code, emoticon){
                        $line.append($('<img />').attr({
                            src: emoticon.url || emoticon, alt: code, title: emoticon.tooltip || code
                        }).click(function (){
                    ...
                    if(!includeMore){
                        $content.append($(editor._('<a class="sceditor-more">{0}</a>', editor._('More'))).click(function (){
        ...
        youtube: {
            _dropDown: function (editor, caller, handleIdFunc){
                var matches, content = _tmpl('youtubeMenu', {
                    label: editor._('Video URL:'), insert: editor._('Insert')
                }, true);
        ...
        date: {
            _date: function (editor){
                var now = new Date(), year = now.getYear(), month = now.getMonth() + 1, day = now.getDate();
        ...
        ltr: {
            state: function (parents, firstBlock){
                ...
                var editor = this, elm = editor.getRangeHelper().getFirstBlockParent(), $elm = $(elm);
    ...
    $.sceditor.rangeHelper = function (w, d){
        var _createMarker, _isOwner, _prepareHTML, doc = d || w.contentDocument || w.document, win = w, isW3C = !!w.getSelection, startMarker = 'sceditor-start-marker', endMarker = 'sceditor-end-marker', characterStr = 'character', // Used to improve minification
            base = this;
        ...
        _prepareHTML = function (node, endNode, returnHtml){
            var lastChild, $lastChild, toInsert, div = doc.createElement('div');
        ...
        base.insertNode = function (node, endNode){
            if(isW3C){
                var toInsert = _prepareHTML(node, endNode), range = base.selectedRange(), parent = range.commonAncestorContainer;
                ...
                if(parent && /^(br|hr)$/i.test(parent.nodeName))
                    parent.parentNode.insertBefore(toInsert, parent);else
        ...
        _isOwner = function (range){
            ...
            // IE fix to make sure only return selections that are part of the WYSIWYG iframe
            return (range && range.parentElement && (parent = range.parentElement())) ? parent.ownerDocument === doc : true;
        };
        ...
        base.hasSelection = function (){
            var range, sel = isW3C ? win.getSelection() : doc.selection;
        ...
        base.insertNodeAt = function (start, node){
            var currentRange = base.selectedRange(), range = base.cloneSelected();
        ...
        base.restoreRange = function (){
            var marker, range = base.selectedRange(), start = base.getMarker(startMarker), end = base.getMarker(endMarker);
        ...
        base.getOuterText = function (before, length){
            var ret = '', range = base.cloneSelected();
        ...
        base.raplaceKeyword = function (keywords, includeAfter, keywordsSorted, longestKeyword, requireWhiteSpace, currrentChar){
            ...
            var beforeStr, str, keywordIdx, numberCharsLeft, keywordRegex, startIdx, keyword, i = keywords.length, maxKeyLen = longestKeyword || keywords[i - 1][0].length;
        ...
        base.compare = function (rangeA, rangeB){
            ...
            return rangeA.compareBoundaryPoints(Range.END_TO_END, rangeB) === 0 && rangeA.compareBoundaryPoints(Range.START_TO_START, rangeB) === 0;
        };
     ...
     */
    $.sceditor.dom = /** @lends jQuery.sceditor.dom */
    {
        /**
        ...
        parseHTML: function (html, context){
            var ret = [], tmp = (context || document).createElement('div');
        ...
        convertElement: function (elm, newElement){
            var child, attr, i = elm.attributes.length, newTag = elm.ownerDocument.createElement(newElement);
        ...
        fixNesting: function (node){
            var base = this, getLastInlineParent = function (node){
            ...
            base.traverse(node, function (node){
                ...
                if(node.nodeType === 1 && !base.isInline(node, true) && base.isInline(node.parentNode, true)){
                    var parent = getLastInlineParent(node), rParent = parent.parentNode, before = base.extractContents(parent, node), middle = node;
        ...
        removeWhiteSpace: function (root, preserveNewLines){
            var nodeValue, nodeType, next, previous, cssWS, nextNode, trimStart, sibling, getSibling = $.sceditor.dom.getSibling, isInline = $.sceditor.dom.isInline, node = root.firstChild, whitespace = /[\t ]+/g, witespaceAndLines = /[\t\n\r ]+/g;
        ...
        extractContents: function (startNode, endNode){
            var base = this, $commonAncestor = base.findCommonAncestor(startNode, endNode), commonAncestor = !$commonAncestor ? null : $commonAncestor[0], startReached = false, endReached = false;
            ...
            return (function extract(root){
                ...
                base.traverse(root, function (node){
                    ...
                    // if this node contains the start node then add it
                        ...
                        if(jQuery.contains(node, startNode) && node.nodeType === 1){
                            c = extract(node);
                            n = node.cloneNode(false);
        ...
        getStyle: function (elm, property){
            var $elm, direction, propertyCache, styleValue, elmStyle = elm.style;
    ...
    $.sceditor.PluginManager = function (owner){
        ...
        var callHandlers = function (args, returnAtFirst){
            ...
            var i = plugins.length, signal = formatSignalName(args.shift());
        ...
        base.deregister = function (plugin){
            var removedPlugin, i = plugins.length, ret = false;
     ...
     */
    $.sceditor.command = /** @lends jQuery.sceditor.command */
    {
        /**
         * Gets a command
         *
    ...
    $.sceditor.defaultOptions = {
         ...
         * @type {String}
         */
        toolbar: 'bold,italic,underline,strike,subscript,superscript|left,center,right,justify|' + 'font,size,color,removeformat,cut,copy,paste,pastetext|bulletlist,orderedlist,indent,outdent|' + 'table,code,quote|horizontalrule,image,email,link,unlink|emoticon,youtube|' + 'ltr,rtl|maximize,source',
        ...
        emoticonsRoot: '', emoticons: {
            dropdown: {
                ...
                ':whistling:': 'emoticons/whistling.png', ':love:': 'emoticons/wub.png'
            }
        },
        ...
        getHtmlHandler: null, getTextHandler: null,
    ...
    $.fn.sceditor = function (options){
        ...
        this.each(function (){
            ...
            if(options === 'state')
                ret.push(!!$this.data('sceditor'));else
                if(options === 'instance')
                    ret.push($this.data('sceditor'));else
...
(function ($, window, document){
    ...
    $.sceditor.BBCodeParser = function (options){
        ...
        TokenizeToken.prototype = {
            ...
            splitAt: function (splitAt){
                var clone, base = this, splitAtLength = 0, childrenLen = base.children.length;
        ...
        base.tokenize = function (str){
            var matches, type, i, toks = [], tokens = [// Close must come before open as they are
        ...
        tokenizeTag = function (type, val){
            ...
            if(type === 'open' && (matches = val.match(/\[([^\]\s=]+)(?:([^\]]+))?\]/))){
                ...
                if(type === 'close' && (matches = val.match(/\[\/([^\[\]]+)\]/)))
                    name = lower(matches[1]);else
                    if(type === 'newline')
                        name = '#newline';
        ...
        tokenizeAttrs = function (attrs){
             ...
             */
                atribsRegex = /([^\s=]+)=(?:(?:(["'])((?:\\\2|[^\2])*?)\2)|((?:.(?!\s\S+=))*.))/g, unquote = $.sceditor.plugins.bbcode.stripQuotes, ret = {};

            // if only one attribute then remove the = from the start and strip any quotes
            if(attrs.charAt(0) === '=' && attrs.indexOf('=', 1) < 0)
                ret.defaultattr = unquote(attrs.substr(1));else{
                if(attrs.charAt(0) === '=')
        ...
        isChildAllowed = function (parent, child){
            var bbcode = parent ? base.bbcodes[parent.name] : null, allowedChildren = bbcode ? bbcode.allowedChildren : null;
        ...
        parseTokens = function (toks){
            var token, bbcode, curTok, clone, i, previous, next, cloned = [], output = [], openTags = [], /**
        ...
        normaliseNewLines = function (children, parent, onlyRemoveBreakAfter){
            var token, left, right, parentBBCode, bbcode, removedBreakEnd, removedBreakBefore, remove, childrenLength = children.length, i = childrenLength;
        ...
        fixNesting = function (children, parents, insideInline, rootArr){
            var token, i, parent, parentIndex, parentParentChildren, right, isInline = function (token){
        ...
        fixChildren = function (children, parent){
            var token, args, i = children.length;
        ...
        removeEmpty = function (tokens){
            var token, bbcode, isTokenWhiteSpace, i = tokens.length;
        ...
        convertToHTML = function (tokens, isRoot){
            var token, bbcode, content, html, needsBlockWrap, blockWrapOpen, isInline, lastChild, ret = [];
        ...
        convertToBBCode = function (toks){
            var token, attr, bbcode, isBlock, isSelfClosing, quoteType, breakBefore, breakStart, breakEnd, breakAfter, // Create an array of strings which are joined together
        ...
        quote = function (str, quoteType, name){
            var QuoteTypes = $.sceditor.BBCodeParser.QuoteType, needsQuotes = /\s|=/.test(str);
     ...
     */
        $.sceditor.plugins.bbcode = function (){
             ...
             * @private
             */
            var buildBbcodeCache, handleStyles, handleTags, formatString, mergeSourceModeCommands, removeFirstLastDiv;
                ...
                var merge = {
                    ...
                    font: {
                        txtExec: function (caller){
                            ...
                            getCommand('font')._dropDown(editor, caller, function (fontName){
                    ...
                    size: {
                        txtExec: function (caller){
                            ...
                            getCommand('size')._dropDown(editor, caller, function (fontSize){
                    ...
                    color: {
                        txtExec: function (caller){
                            ...
                            getCommand('color')._dropDown(editor, caller, function (color){
                    ...
                    email: {
                        txtExec: function (caller, selected){
                            var display = selected && selected.indexOf('@') > -1 ? null : selected, email = prompt(this._('Enter the e-mail address:'), (display ? '' : selected)), text = prompt(this._('Enter the displayed text:'), display || email) || email;
                    ...
                    link: {
                        txtExec: function (caller, selected){
                            var display = /^[a-z]+:\/\//i.test($.trim(selected)) ? null : selected, url = prompt(this._('Enter URL:'), (display ? 'http://' : $.trim(selected))), text = prompt(this._('Enter the displayed text:'), display || url) || url;
                    ...
                    youtube: {
                        txtExec: function (caller){
                            ...
                            getCommand('youtube')._dropDown(editor, caller, function (id){
                    ...
                    $.each(bbcodes, function (bbcode, values){
                        if(!values || $.inArray(styleValue.toString(), values) > -1){
                            ...
                            if($.isFunction(format))
                                content = format.call(base, $element, content);else
            ...
            handleTags = function ($element, content, blockLevel){
                ...
                if(blockLevel && (!isInline(element, true) || tag === 'br')){
                    var parent = element.parentNode, parentLastChild = parent.lastChild, previousSibling = element.previousSibling, parentIsInline = isInline(parent, true);

                    // skips selection makers and other ignored items and empty inlines
                    while(previousSibling && ($(previousSibling).hasClass('sceditor-ignore') || (previousSibling.nodeType === 1 && isInline(previousSibling, true) && !previousSibling.firstChild))){
            ...
            base.signalToSource = function (html, $body){
                var $tmpContainer, bbcode, parser = new $.sceditor.BBCodeParser(base.opts.parserOptions);
            ...
            base.elementToBbcode = function ($element){
                return (function toBBCode(node, vChildren){
                    ...
                    $.sceditor.dom.traverse(node, function (node){
                        var $node = $(node), curTag = '', nodeType = node.nodeType, tag = node.nodeName.toLowerCase(), vChild = validChildren[tag], firstChild = node.firstChild, isValidChild = true;
                        ...
                        if(nodeType === 1){
                            ...
                            if(node.wholeText && (!node.previousSibling || node.previousSibling.nodeType !== 3)){
            ...
            base.signalToWysiwyg = function (text, asFragment){
                var parser = new $.sceditor.BBCodeParser(base.opts.parserOptions), html = parser.toHTML(base.opts.bbcodeTrim ? $.trim(text) : text);
    ...
    $.sceditor.plugins.bbcode.bbcodes = {
        ...
        b: {
            ...
            }, format: '[b]{0}[/b]', html: '<strong>{0}</strong>'
        }, // END_COMMAND
        ...
        sub: {
            tags: {
                sub: null
            }, format: '[sub]{0}[/sub]', html: '<sub>{0}</sub>'
        }, // END_COMMAND
        ...
        sup: {
            tags: {
                sup: null
            }, format: '[sup]{0}[/sup]', html: '<sup>{0}</sup>'
        }, // END_COMMAND
        ...
        font: {
            tags: {
                ...
                'font-family': null
            }, quoteType: $.sceditor.BBCodeParser.QuoteType.never, format: function (element, content){
                var font;
        ...
        size: {
            tags: {
                ...
                var fontSize = element.attr('size'), size = 2;
        ...
        color: {
            tags: {
                ...
                return '<font color="' + escapeEntities(normaliseColour(attrs.defaultattr), true) + '">' + content + '</font>';
            }
        }, // END_COMMAND
        ...
        ul: {
            tags: {
                ul: null
            }, breakStart: true, isInline: false, skipLastLineBreak: true, format: '[ul]{0}[/ul]', html: '<ul>{0}</ul>'
        }, list: {
            breakStart: true, isInline: false, skipLastLineBreak: true, html: '<ul>{0}</ul>'
        }, ol: {
            tags: {
                ol: null
            }, breakStart: true, isInline: false, skipLastLineBreak: true, format: '[ol]{0}[/ol]', html: '<ol>{0}</ol>'
        }, li: {
            tags: {
                li: null
            },
            ...
            isInline: false, closedBy: ['/ul', '/ol', '/list', '*', 'li'], html: '<li>{0}</li>'
        }, // END_COMMAND
        ...
        table: {
            ...
            tags: {
                tr: null
            }, isInline: false, skipLastLineBreak: true, format: '[tr]{0}[/tr]', html: '<tr>{0}</tr>'
        }, th: {
            tags: {
                th: null
            }, allowsEmpty: true, isInline: false, format: '[th]{0}[/th]', html: '<th>{0}</th>'
        }, td: {
            tags: {
                td: null
            }, allowsEmpty: true, isInline: false, format: '[td]{0}[/td]', html: '<td>{0}</td>'
        }, // END_COMMAND

        // START_COMMAND: Emoticons
        emoticon: {
            allowsEmpty: true, tags: {
                img: {
                    src: null, 'data-sceditor-emoticon': null
                }
        ...
        hr: {
            tags: {
                hr: null
            }, allowsEmpty: true, isSelfClosing: true, isInline: false, format: '[hr]{0}', html: '<hr />'
        }, // END_COMMAND
        ...
        img: {
            ...
            format: function ($element, content){
                var w, h, attribs = '', element = $element[0], style = function (name){
        ...
        // START_COMMAND: E-mail
        email: {
            quoteType: $.sceditor.BBCodeParser.QuoteType.never, html: function (token, attrs, content){
        ...
        quote: {
            tags: {
                blockquote: null
            }, isInline: false, quoteType: $.sceditor.BBCodeParser.QuoteType.never, format: function (element, content){
        ...
        code: {
            tags: {
                code: null
            }, isInline: false, allowedChildren: ['#', '#newline'], format: '[code]{0}[/code]', html: '<code>{0}</code>'
        }, // END_COMMAND
        ...
        left: {
            ...
            }, isInline: false, format: '[left]{0}[/left]', html: '<div align="left">{0}</div>'
        }, // END_COMMAND
        ...
        center: {
            ...
            }, isInline: false, format: '[center]{0}[/center]', html: '<div align="center">{0}</div>'
        }, // END_COMMAND
        ...
        right: {
            ...
            }, isInline: false, format: '[right]{0}[/right]', html: '<div align="right">{0}</div>'
        }, // END_COMMAND
        ...
        justify: {
            ...
            }, isInline: false, format: '[justify]{0}[/justify]', html: '<div align="justify">{0}</div>'
        }, // END_COMMAND
        ...
        youtube: {
            ...
            html: '<iframe width="560" height="315" src="https://www.youtube.com/embed/{0}?wmode=opaque' + '" data-youtube-id="{0}" frameborder="0" allowfullscreen></iframe>'
        }, // END_COMMAND
        ...
        rtl: {
            styles: {
                'direction': ['rtl']
            }, format: '[rtl]{0}[/rtl]', html: '<div style="direction: rtl">{0}</div>'
        }, // END_COMMAND
        ...
        ltr: {
            styles: {
                'direction': ['ltr']
            }, format: '[ltr]{0}[/ltr]', html: '<div style="direction: ltr">{0}</div>'
        }, // END_COMMAND
     ...
     */
    $.sceditor.plugins.bbcode.bbcode = /** @lends jQuery.plugins.bbcode.bbcode */
    {
        /**
         * Gets a BBCode
         *
