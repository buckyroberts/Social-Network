...
(function ($, window, document){
    ...
    $.sceditor = function (el, options){
        ...
        base.createDropDown = function (menuItem, dropDownName, content, ieUnselectable){
            ...
            dropDownCss = {
                top: menuItem.offset().top, left: menuItem.offset().left, marginTop: menuItem.outerHeight()
            };
...
(function ($){
    ...
    $.sceditor.XHTMLSerializer = function (){
         ...
         * @private
         */
        var escapeEntites, trim, serializeNode, handleDoc, handleElement, handleCdata, handleComment, handleText, output, canIndent;
         ...
         */
        trim = function (str){
            return str// new lines in text nodes are always ignored in normal handling
                .replace(/[\r\n]/, '').replace(/[^\S|\u00A0]+/g, ' ');
        };
        ...
        handleElement = function (node, parentIsPre){
            var child, attr, attrValue, tagName = node.nodeName.toLowerCase(), i = node.attributes.length, // pre || pre-wrap with any vendor prefix
                isPre = parentIsPre || /pre(?:\-wrap)?$/i.test($(node).css('whiteSpace')), selfClosing = !node.firstChild && $.sceditor.XHTMLSerializer.emptyTags.indexOf('|' + tagName + '|') > -1;
            ...
            while(i--){
                ...
                if(!$.sceditor.ie || attr.specified){
                    // IE < 8 doesn't return the CSS for the style attribute
                    if($.sceditor.ie < 8 && /style/i.test(attr.name))
                        attrValue = node.style.cssText;else
    ...
    $.sceditor.plugins.xhtml = function (){
         ...
         * @private
         */
        var mergeSourceModeCommands, convertTags, convertNode, isEmpty, removetags, mergeAttribsFilters, removeAttribs;
        ...
        mergeSourceModeCommands = function (editor){
            var merge = {
                ...
                link: {
                    txtExec: function (caller, selected){
                        var display = selected && selected.indexOf('http://') > -1 ? null : selected, url = prompt(this._('Enter URL:'), (display ? 'http://' : selected)), text = prompt(this._('Enter the displayed text:'), display || url) || url;
                ...
                youtube: {
                    txtExec: function (caller){
                        ...
                        $.sceditor.command.get('youtube')._dropDown(editor, caller, function (id){
                            editor.insertText('<iframe width="560" height="315" src="https://www.youtube.com/embed/{id}?wmode=opaque" data-youtube-id="' + id + '" frameborder="0" allowfullscreen></iframe>');
                        });
                    }
                },
        ...
        convertTags = function (node){
            $.sceditor.dom.traverse(node, function (node){
                var $node = $(node), tagName = node.nodeName.toLowerCase();
        ...
        isEmpty = function (node, excludeBr){
            var childNodes = node.childNodes, tagName = node.nodeName.toLowerCase(), nodeValue = node.nodeValue, childrenLength = childNodes.length;
        ...
        removetags = function (node){
            $.sceditor.dom.traverse(node, function (node){
                var remove, tagName = node.nodeName.toLowerCase(), empty = tagName !== 'iframe' && isEmpty(node), parentNode = node.parentNode, nodeType = node.nodeType, allowedtags = $.sceditor.plugins.xhtml.allowedTags, disallowedTags = $.sceditor.plugins.xhtml.disallowedTags;
                ...
                // 3 is text node which do not get filtered
                    ...
                    if(allowedtags && allowedtags.length)
                        remove = ($.inArray(tagName, allowedtags) < 0);else
                        if(disallowedTags && disallowedTags.length)
        ...
        mergeAttribsFilters = function (filtersA, filtersB){
            ...
            $.each(filtersB, function (attrName, values){
                if($.isArray(values))
                    ret[attrName] = $.merge(ret[attrName] || [], values);else
                    if(!ret[attrName])
                        ret[attrName] = null;
            });
        ...
        removeAttribs = function (node){
            var tagName, attr, attrName, attrsLength, validValues, remove, allowedAttribs = $.sceditor.plugins.xhtml.allowedAttribs, isAllowed = allowedAttribs && !$.isEmptyObject(allowedAttribs), disallowedAttribs = $.sceditor.plugins.xhtml.disallowedAttribs, isDisallowed = disallowedAttribs && !$.isEmptyObject(disallowedAttribs);
            ...
            $.sceditor.dom.traverse(node, function (node){
                ...
                if(attrsLength){
                    if(!attrsCache[tagName]){
                        if(isAllowed)
                            attrsCache[tagName] = mergeAttribsFilters(allowedAttribs['*'], allowedAttribs[tagName]);else
                    ...
                    while(attrsLength--){
                        ...
                        if(isAllowed)
                            remove = validValues !== null && (!$.isArray(validValues) || $.inArray(attr.value, validValues) < 0);else
                            if(isDisallowed)
    ...
    $.sceditor.plugins.xhtml.converters = [{
        ...
        tags: {
            ...
            if($.sceditor.ie < 8)
                node.removeAttribute('value');else
        ...
        tags: {
            ...
            $node.css('marginTop', $node.attr('vspace') - 0).css('marginBottom', $node.attr('vspace') - 0).removeAttr('vspace');
        }
    }, {
        tags: {
            '*': {
            ...
            $node.css('marginLeft', $node.attr('hspace') - 0).css('marginRight', $node.attr('hspace') - 0).removeAttr('hspace');
        }
    }, {
        tags: {
            'hr': {
        ...
        tags: {
            ...
            var size = $node.css('fontSize'), fontSize = size;
