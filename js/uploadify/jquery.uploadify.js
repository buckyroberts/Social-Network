...
(function ($){
        ...
        init: function (options, swfUploadOptions){
            ...
            return this.each(function (){
                ...
                if(flashInstalled){
                    ...
                    // Wrap the instance
                    var $wrapper = $('<div />', {
                        'id': settings.id, 'class': 'uploadify', 'css': {
                            'height': settings.height + 'px', 'width': settings.width + 'px'
                        }
                    });
                    ...
                    // Create the button
                    var $button = $('<div />', {
                        'id': settings.id + '-button', 'class': 'uploadify-button ' + settings.buttonClass
                    });
                    if(settings.buttonImage){
                        $button.css({
                            'background-image': "url('" + settings.buttonImage + "')", 'text-indent': '-9999px'
                        });
                    }
                    ...
                    if(!settings.queueID){
                        var $queue = $('<div />', {
                            'id': settings.id + '-queue', 'class': 'uploadify-queue'
                        });
                        $wrapper.after($queue);
    ...
    var handlers = {
        ...
        onSelect: function (file){
            ...
            itemData = {
                'fileID': file.id, 'instanceID': settings.id, 'fileName': fileName, 'fileSize': fileSize
            }
        ...
        onUploadProgress: function (file, fileBytesLoaded, fileTotalBytes){
            ...
            if($.inArray('onUploadProgress', settings.overrideEvents) < 0){
                if(settings.progressData == 'percentage'){
                    ...
                    if(settings.progressData == 'speed' && lapsedTime > 500){
